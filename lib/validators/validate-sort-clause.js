 ...
 *
 * @param  {String|Array|Dictionary} sort
...
module.exports = function validateSortClause(sort) {
  ...
  if (_.isString(sort)) {
    ...
    // If the array is empty, then this is a bit strange.
    // e.g. `{ sort: [] }`
    ...
    throw flaverr('E_SORT_CLAUSE_UNPARSEABLE', new Error(E_SORT_CLAUSE_UNPARSEABLE_MSG_PREFIX + 'Expected `sort` to be a string, array, or dictionary, but instead got: `'+util.inspect(sort,{depth: null})+'`'));
  }
